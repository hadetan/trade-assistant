...
- **UI:** manual verification in the running app (per the `verify` skill) for the chat flow, the mandatory AI/Engine-Only session prompt, both auth banners, the Engine-Only question wizard, and a proactive-scan tick — automated UI testing is not the priority for a single-user personal tool, but the golden path must be manually driven at least once per milestone.
...
8. Decide the exact historical-replay harness UX (§10.3) at implementation time — e.g. whether it's a CLI-only dev tool or gets a lightweight UI panel of its own.
