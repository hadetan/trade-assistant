...
Live protocol testing against the production Kite MCP endpoint (`https://mcp.kite.trade/mcp`, server self-reporting `v0.3.2`) on 2026-07-18 showed it exposes 24 tools, including `place_order`, `modify_order`, `cancel_order`, `place_gtt_order`, `modify_gtt_order`, `delete_gtt_order` — gated by the same generic login-session check as every read tool, with no "tool disabled" response. This directly contradicts Zerodha's own README (`github.com/zerodha/kite-mcp-server`) and support docs, which claim the hosted instance excludes destructive operations by default via a server-side `EXCLUDED_TOOLS` config flag. Two independent research/verify passes confirmed this by directly querying the live server, not by trusting its documentation. This also settles the earlier open question of whether GTT write tools are excluded separately from the three plain order tools: they were not — all six were present together in the same observed `tools/list` response, so there is no partial-exclusion case to design around, only "assume none of the six are ever safe to call, regardless of what the hosted config currently does."

Note for implementation sequencing: `tools/list` is a protocol-level introspection call and does **not** require an authenticated Kite session — only actually *calling* a tool (`tools/call`) against account-specific data does (an unauthenticated call returns a functional "please log in first" response, not a protocol error). This means the drift-detection audit below (layer 3) can be built and exercised before the user has completed Kite Connect account setup/first login at all — it doesn't wait on that.
...
- Volume: OBV, session-anchored VWAP (9:15 IST reset), MFI, CMF, Accumulation/Distribution, and **Volume Profile** — confirmed feasible from Kite's 1-minute OHLCV bars alone (no tick data needed): distribute each bar's volume as a uniform density across its `[Low, High]` range, then accumulate into price bins by overlap-fraction (not close-only or equal-split-per-touched-bin, both documented bugs in public implementations). This is mechanically the same technique TradingView, Sierra Chart, NinjaTrader, MotiveWave, and MultiCharts all use once tick data isn't available — the accuracy comes from using fine (1-minute) bars, not from having true tick data.
...
**Kite OAuth redirect capture (resolved):** a short-lived local loopback HTTP server, combined with the OS's real default system browser for the login itself — not an embedded `BrowserWindow`. Kite Connect's developer console explicitly accepts `http://127.0.0.1:<port>/...` as a registered `redirect_url` ("you can use a `http://127.0.0.1` URL for testing" — Kite's own API FAQ; confirmed by Zerodha staff on the developer forum, and by Zerodha's own `kite-mcp-server` reference project using exactly this pattern, `http://localhost:3000/callback`, for its non-hosted mode). On "Login to Kite," Electron main starts a plain `http.createServer` on the registered port and opens the login URL via `shell.openExternal` in the user's actual system browser (not an in-app view) — the system browser means the user's password manager/2FA autofill work normally, avoiding the documented fragility of embedded-browser logins (Electron's own docs discourage `<webview>`; other providers have been known to flag embedded browser contexts as insecure). Zerodha redirects the system browser to the loopback URL with `request_token` in the query string; the listener's one-shot handler captures it, shows a minimal "you can close this tab" page, and shuts itself down. The `access_token` exchange (§5.1) proceeds from there, once per day per the mandatory daily re-login.
...
- **Community archives** — a Kaggle dataset family (`debashis74017/...`, Nifty 50/100/500 + Bank Nifty, 1/5/15-minute, 2015→present, apparently updated regularly) and a static GitHub archive (`aeron7/nifty-banknifty-intraday-data`, 1-minute OHLCV across ~150 F&O stocks/futures/sector indices/VIX, 2007/2012 through Feb 2023, then frozen). Both are free and need no authentication. Licensing was checked directly rather than assumed, with a real difference between the two: **the Kaggle family is explicitly CC0 (public domain)**, confirmed via Kaggle's own dataset API for every dataset in that uploader's catalog — clean, unambiguous, no restriction on this app's personal/non-redistributed use, modulo a soft "research purpose only" disclaimer in the listing's own description (worth respecting in spirit, not a formal restriction). **The GitHub archive has no actual license grant** — its README claims MIT but links to a `LICENSE` file that does not exist anywhere in the repo or its commit history, so it is technically all-rights-reserved by default. Practical risk for a private, local, non-redistributed dev/test use is low given the maintainer's stated (if informally backed) intent, but treat the Kaggle CC0 dataset as the preferred/primary source and the GitHub archive as a lower-confidence supplement, not something to rely on as cleanly licensed. Neither source has an SLA or a guaranteed-continuous maintainer, and neither has an independently-audited data-quality/corporate-action-adjustment story.
...
**Practical use:** the Kaggle + `aeron7` GitHub combination is real, free, multi-year, sub-daily data — good enough to seed and validate the intraday side of the backtest/replay harness during development. It is explicitly **not** treated as a long-term or production data dependency: no continuity guarantee (GitHub archive in particular), no SLA. **From the moment the app is live on a real Kite Connect subscription, every candle it fetches is also written into the same Parquet lake (§5.3), permanently** — this is core v1 behavior, not a someday-maybe extension. The point is exactly what it sounds like: build a first-party, fully-owned intraday archive from real usage starting on day one of live use, so reliance on the community datasets is purely a bootstrapping measure for the period before that archive exists, and shrinks on its own as real history accumulates (§16).
...
**Decision-point sampling — reused from live behavior, not an arbitrary stride.** Two default cadences, selectable in setup, both real engineering choices rather than a placeholder:
- **Session-close, for positional/swing benchmarking:** one decision point per trading day (each new daily candle), matching bhavcopy's natural granularity (§10.1) and giving a clean day-over-day hit-rate.
- **Live-equivalent gating, for intraday benchmarking:** reuses the exact same deterministic pre-response gate from §8.1 (confluence delta exceeds a threshold, an IV spike, an OI-buildup flip) to decide which candles become decision points — so a benchmark's cadence matches precisely what the live proactive scanner would actually have evaluated, rather than an idealized dense/uniform stride no real usage would replicate. This makes the resulting hit-rate directly meaningful as a prediction of live behavior, not just a number computed at a sampling density chosen for convenience.
- A manual "every candle" / "every N candles" override is also available for exploratory/debugging use (e.g. stress-testing one algorithm densely), but the two cadences above are the defaults and the ones the headline hit-rate number is computed from.
...
Most of the questions raised in earlier revisions of this doc were run down with direct research rather than left open — the GTT-tool-exclusion question (resolved in §4), the OAuth redirect mechanism (resolved in §8.3), Volume Profile feasibility (resolved in §6.2), the two community-archive licenses (resolved in §10.2), and the historical-replay/Benchmark-UI sampling strategy (resolved in §10.4). What's left genuinely open:

1. **Kronos → ONNX export feasibility** (§6.2) — early spike, before building the rest of the algorithm catalog around the assumption it works. Contingency is already decided either way: if the export works, build it into the Rust sidecar via `ort`; if not, fall back to a small supervised Python/ONNX sidecar process, same architecture shape either way.
2. **Exact current per-interval historical-data lookback caps** — the community-sourced numbers in §5.1 aren't on Kite's current official docs page; validate them empirically against the live API when building the historical-data client, rather than hardcoding pagination logic around unverified figures.
...
- Recording live Kite-sourced candles into the Parquet lake is core v1 behavior, not deferred (§10.2) — the actual future milestone is the point at which the self-recorded archive is deep enough that the Benchmark UI (§10.4) no longer needs the community-archive datasets at all for intraday history, even for bootstrapping.
