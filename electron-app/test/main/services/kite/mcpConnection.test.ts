...
import { connectKiteMcp, connectKiteMcpAnonymous } from "../../../../src/main/services/kite/mcpConnection";
...
describe("connectKiteMcpAnonymous", () => {
  it("connects with no auth headers, against the default url", async () => {
    const client = fakeClient();
    const createClient = vi.fn().mockResolvedValue(client);

    await connectKiteMcpAnonymous({ createClient });

    expect(createClient).toHaveBeenCalledWith({ url: "https://mcp.kite.trade/mcp" });
  });

  it("adapts callTool and listTools through mcpClientAdapter", async () => {
    const client = fakeClient();
    const conn = await connectKiteMcpAnonymous({ createClient: async () => client });

    await conn.caller.callTool("login", {});
    expect(client.callTool).toHaveBeenCalledWith({ name: "login", arguments: {} });
    expect(await conn.listing.listTools()).toEqual(["login", "get_ltp"]);
  });
  ...
  it("honours a custom url", async () => {
    const createClient = vi.fn().mockResolvedValue(fakeClient());
    await connectKiteMcpAnonymous({ url: "https://example.test/mcp", createClient });
    expect(createClient).toHaveBeenCalledWith({ url: "https://example.test/mcp" });
  });

  it("defaults to an empty deps object", async () => {
    const createClient = vi.fn().mockResolvedValue(fakeClient());
    await connectKiteMcpAnonymous({ createClient });
    expect(createClient).toHaveBeenCalledWith({ url: "https://mcp.kite.trade/mcp" });
  });
